pro m_fords,im,swid,orc,ome,minreq=minreq,trace=trace
...
if n_params() lt 3 then begin
   print,'syntax: m_fords,im,swid,orc[,ome].'
   retall
endif
...
;  ACTUALLY DECREASE.
  ...
  if trc then print,'FORDS: Degree of polynomial fit to order locations = ' $
...
;Define useful quantities.
  ...
  if trc then print,'FORDS: Number of swaths = ' + strtrim(string(nswa),2)
  if trc then print,'FORDS: Number of cols,rows = ' + $
...
;Determine positions of order peaks and verify their validity.
  if trc then print,'FORDS: Estimating positions of order peaks in first swath.'
...
;
  ...
  m_fndpks,swa,pk				;get order peaks
  if not keyword_set(pk) then begin        	;check for error in FNDPKS
    orc = 0					;scalar orc sets error flag
    if trc then print,'FORDS: Invalid order peaks in swath - returning without ORCs.'
...
;end
  ...
  if trc then print,'FORDS: Number of peaks found = ' + strtrim(string(nord),2)
  if nord lt minreq then begin			;true: too few orders found
    orc = 0					;scalar orc flags error
    if trc then print,'FORDS: Too few orders found in initial swath' $
...
;  with poor peak determinations are left zero.
  if trc then print,'FORDS: Mapping entire image.  Be patient....'
...
;  then return with orc set to scalar zero, flagging error condition.
  if trc then print,'FORDS: Fitting polynomials to order peaks.'
...
;   print,'Order:',ior,'  # misses:',nmiss
	...
	goto,jump1				;ignore problems near edges
      ...
      if trc then print,'FORDS: ' + fstr + '% of peaks in order missing.'
      if trc then print,'FORDS: Too many missing peaks - returning without orcs.'
      return
    endif
    ...
    if ome[ior] gt maxome then begin		;orc mean error too large?
      if trc then print,'FORDS: Excessive scatter in peaks - returning without orcs.'
      orc = 0					;scalar zero flags error
      return
    endif
...
;   if trc then print,'FORDS: Trimming last order - too few peaks.'
; endif
...
;   if trc then print,'FORDS: Trimming first order - too few peaks.'
; endif
;		  END
...
;   ome = ome[0:nord-2]				;remove last error point
;   nord = nord - 1				;decrement order count
;   if trc then print,'FORDS: Trimming last order - off edge of image.'
; end
...
;   ome = ome[1:nord-1]				;remove first error point
;   nord = nord - 1				;decrement order count
;   if trc then print,'FORDS: Trimming first order - off edge of image.'
; endif

;Order coefficients determined.
  if trc then print,'FORDS: Total missing peaks = ' + strtrim(string(tomiss),2)
  if trc then print,''
  if trc then print,'***************************'
  if trc then print,'FORDS: Orders found = ' + strtrim(string(nord),2)
  if trc then print,'            Should be 16.'
  if trc then print,'***************************'
  if trc then print,''
  wait,1
  if nord lt minreq then begin			;true: too few orders found
